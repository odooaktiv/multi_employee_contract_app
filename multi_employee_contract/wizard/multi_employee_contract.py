...
#     create selected employee's contract
    ...
    def multi_employee_contract(self):
        ...
        contract_id = self.env['hr.contract'].search([('employee_id', 'in' , [emp.employee_id.id for emp in self.employee_ids]), ('state', 'not in',  ['close'])], order="date_start desc")
        if contract_id:
            raise Warning (_('Contract already exist of employee = %s.') % ",".join([contract.employee_id.name for contract in contract_id]))
        for emp_id in self.employee_ids:
            contract_vals = self.env['hr.contract'].create({'name': emp_id.employee_id.name + '\'s contract',
